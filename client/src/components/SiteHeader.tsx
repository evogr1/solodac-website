...
export default function SiteHeader({ activeSection }: { activeSection?: string }) {
  ...
  return (
    <header className="sticky top-2 z-50 bg-transparent px-2 pt-2 sm:top-3 sm:px-4 sm:pt-3">
      <div className="mx-auto flex w-full max-w-[1440px] items-center justify-between gap-3 md:w-fit md:justify-normal">
        <div
          className={`flex items-center rounded-full border border-white/10 bg-black/40 shadow-[0_18px_60px_rgba(0,0,0,.28)] backdrop-blur-xl transition-all duration-300 ${
            scrolled ? "min-h-[40px] gap-0 px-2 py-1 md:gap-0" : "min-h-[52px] gap-3 px-3 py-1.5 sm:min-h-[56px] sm:px-4 md:gap-10 lg:gap-14 lg:px-6"
          }`}
        >
        ...
        <button
          ...
          className={`hidden items-center gap-2 rounded-full border border-white/30 bg-black/40 text-[11px] font-bold uppercase tracking-[0.14em] text-paper shadow-[0_18px_60px_rgba(0,0,0,.28)] shadow-inner backdrop-blur-xl transition-all duration-300 hover:border-lime hover:bg-lime hover:text-ink md:flex ${
